...
A \textit{Spacer} is used in a \lstinline!GridLayoutPane! to mark an empty cell or in a \lstinline!FlowLayoutPane! to occupy some space. Using an optional additional parameter the actual number of spacers can be specified\todo{Rephrase, do you mean the number of spacers or the size of a single spacer. Furthermore, is space occupied horizontally, vertically or both?}.
\begin{lstlisting}
Spacer (INT)
\end{lstlisting}
